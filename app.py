...
def chatbot():
    ...
    try:
        ...
        if user_input == "이전":
            ...
            # 영양표시 연령 선택 모드에서 이전 -> 영양성분검사 메뉴로
            if user_data.get("영양표시_연령선택"):
                user_data.pop("영양표시_연령선택", None)
...
🔽 사용하고자 하는 용어를 선택해주세요""",
                ...
                age_label = "영유아(만1~2세)" if age == "영유아" else "일반(3세 이상)"
                return make_response(
                    f"📊 영양표시 도우미 [{age_label}]\n\n원하시는 기능을 선택해주세요.",
...
다시 계산하시려면 [당알코올 계산]을 선택하세요."""
        ...
        # ===== 영양성분검사 > 영양표시 도우미 선택 시 연령구분 선택 =====
        if user_data.get("검사분야_메뉴") == "영양성분검사" and user_input == "영양표시 도우미":
            # 이미 연령구분이 선택되어 있으면 바로 메뉴 표시
            if user_data.get("영양표시_연령구분"):
                user_data["현재_메뉴"] = "영양표시 도우미"
                age = user_data["영양표시_연령구분"]
                age_label = "영유아(만1~2세)" if age == "영유아" else "일반(3세 이상)"
                return make_response(
                    f"📊 영양표시 도우미 [{age_label}]\n\n원하시는 기능을 선택해주세요.",
                    ["강조표시 확인", "기준치 조회", "표시값 변환", "이전", "처음으로"]

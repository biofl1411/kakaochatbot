...
INSPECTION_MENU = {
    ...
    "submenus": {
        ...
        "영양표시 도우미": {
            "title": "영양표시 도우미",
            "buttons": ["표시단위 계산", "강조표시 확인", "기준치 조회", "표시값 변환", "이전", "처음으로"],
            "parent": "영양성분검사"
        },
        "소비기한설정": {
            "title": "소비기한설정",
...
━━━━━━━━━━━━━━━""",
                ...
                # Q&A 단계(무가당/감미료/저열량/무가염) -> 영양소 선택으로
                if step in ["무가당_조건확인", "감미료_확인", "저열량_확인", "무가염_조건확인"]:
                    claim_type = user_data.get("강조표시_용어")
                    nutrients = user_data.get("강조표시_영양소_목록", [])
                    user_data["강조표시_단계"] = "영양소_선택"
                    user_data.pop("강조표시_영양소", None)
                    user_data.pop("강조표시_함량", None)
                    user_data.pop("감미료_사용", None)
                    return make_response(
                        f"""📊 [{claim_type}] 강조표시
...
🔽 사용하고자 하는 용어를 선택해주세요""",
                ...
                return make_response(
                    "📊 영양표시 도우미\n\n원하시는 기능을 선택해주세요.",
                    ["표시단위 계산", "강조표시 확인", "기준치 조회", "표시값 변환", "이전", "처음으로"]
                )
            ...
            # 표시값 변환 모드에서 이전 -> 단계별 네비게이션
            if user_data.get("표시값변환_모드"):
                step = user_data.get("표시값변환_단계")

                # 산출 관련 state 정리 헬퍼
                def _clear_calc_state():
                    user_data.pop("표시값변환_산출_데이터", None)
                    user_data.pop("표시값변환_산출_인덱스", None)
                    user_data.pop("표시값변환_기타_분류", None)

                # 연령 선택 단계에서 이전 → 영양표시 도우미로
                if step == "연령_선택":
                    user_data.pop("표시값변환_모드", None)
                    user_data.pop("표시값변환_단계", None)
                    user_data.pop("영양표시_연령구분", None)
                    user_data["현재_메뉴"] = "영양표시 도우미"
                    return make_response(
                        "📊 영양표시 도우미\n\n원하시는 기능을 선택해주세요.",
                        ["표시단위 계산", "강조표시 확인", "기준치 조회", "표시값 변환", "이전", "처음으로"]
                ...
                # 열량/탄수화물 입력방식 선택에서 이전 → 영양소 선택으로
                if step in ["열량_입력방식", "탄수화물_입력방식"]:
...
━━━━━━━━━━━━━━━
🔽 영양소를 선택해주세요""",
                        ["열량", "탄수화물", "당류", "식이섬유", "단백질", "지방", "포화지방", "트랜스지방", "콜레스테롤", "나트륨", "기타", "이전", "처음으로"]
                    )

                # 열량 산출유형에서 이전 → 열량 입력방식으로
                if step == "열량_산출유형":
                    user_data["표시값변환_단계"] = "열량_입력방식"
                    _clear_calc_state()
                    return make_response(
                        """📊 [열량] 표시값 변환
...
📌 입력 방식을 선택해주세요
━━━━━━━━━━━━━━━
• 실측값 입력: 열량 실측값을 직접 입력
• 표시값으로 산출: 탄수화물/단백질/지방 표시값으로 열량 계산""",
                        ["실측값 입력", "표시값으로 산출", "이전", "처음으로"]
                    )

                # 기타 분류 선택에서 이전 → 영양소 선택으로
                if step == "기타_분류_선택":
                    user_data["표시값변환_단계"] = "영양소_선택"
                    user_data.pop("표시값변환_기타_분류", None)
                    return make_response(
...
🔽 영양소를 선택해주세요""",
                ...
                # 기타 영양소 선택에서 이전 → 기타 분류 선택으로
                if step == "기타_영양소_선택":
                    user_data["표시값변환_단계"] = "기타_분류_선택"
                    user_data.pop("표시값변환_기타_분류", None)
                    return make_response(
                        """📊 기타 영양소 분류
...
━━━━━━━━━━━━━━━
🔽 분류를 선택해주세요""",
                        ["지방산류", "비타민류", "무기질류", "이전", "처음으로"]
                    )

                # 열량 일반 산출/별도표시/알콜유기산 입력 중 이전 → 영양소 선택으로
                if step in ["열량_일반_입력", "열량_별도_입력", "열량_추가여부", "열량_알콜유기산_입력"]:
                    user_data["표시값변환_단계"] = "영양소_선택"
                    user_data.pop("표시값변환_영양소", None)
                    _clear_calc_state()
                    return make_response(
...
🔽 영양소를 선택해주세요""",
                ...
                # 함량 입력(실측값 직접) 단계에서 이전 → 해당 영양소의 입력방식 또는 영양소 선택으로
                if step == "함량_입력":
...
📌 입력 방식을 선택해주세요
━━━━━━━━━━━━━━━
• 실측값 입력: 탄수화물 실측값을 직접 입력
• 차감법으로 산출: 100g-(수분+단백질+지방+회분)""",
                            ["실측값 입력", "차감법으로 산출", "이전", "처음으로"]
                        )
                    else:
                        user_data["표시값변환_단계"] = "영양소_선택"
                        user_data.pop("표시값변환_영양소", None)
...
🔽 영양소를 선택해주세요""",
        ...
        # ===== 검사분야 하위 메뉴 처리 =====
        # 영양표시 도우미: 연령구분 선택 로직 별도 처리
        # 기타: 표시값변환 모드에서는 기타 영양소 분류로 별도 처리
        if (user_input in INSPECTION_MENU["submenus"]
                and user_input != "영양표시 도우미"
                and not (user_input == "기타" and user_data.get("표시값변환_모드"))):
...
다시 계산하시려면 [당알코올 계산]을 선택하세요."""
        ...
        # ===== 영양성분검사 > 영양표시 도우미 > 표시단위 계산 선택 시 (대화형) =====
        if user_input == "표시단위 계산" and (user_data.get("검사분야_메뉴") == "영양성분검사" or user_data.get("현재_메뉴") == "영양표시 도우미"):
...
숫자만 입력 (단위 제외)""",
            ...
            if step == "함량_입력":
                try:
                    ...
                    # ===== 무가당(당류): 다) 조건 확인 + 마) 추가표시 Q&A =====
                    if claim_type == "무가당" and nutrient == "당류" and target_claim:
                        threshold = target_claim['threshold']
                        if threshold is None or amount < threshold:
                            user_data["강조표시_함량"] = amount
                            user_data["강조표시_단계"] = "무가당_조건확인"
                            return make_response(
                                f"""📊 무가당 추가 조건 확인

━━━━━━━━━━━━━━━
📌 입력 함량: {amount}g (100g당)
   → 함량 기준 충족 (0.5g미만)
━━━━━━━━━━━━━━━
...
🔽 위 (1)~(5) 조건을 모두
   충족하나요?""",
                    ...
                    # ===== 무(당류): 마) 추가 표시사항 Q&A =====
                    if claim_type == "무" and nutrient == "당류" and target_claim:
                        threshold = target_claim['threshold']
                        if threshold is None or amount < threshold:
                            user_data["강조표시_함량"] = amount
                            user_data["강조표시_단계"] = "감미료_확인"
                            return make_response(
                                f"""📊 무(당류) 추가 표시사항 확인
...
🔽 감미료를 사용하나요?""",
                    ...
                    if claim_type == "무첨가" and nutrient == "나트륨":
                        user_data["강조표시_함량"] = amount
                        user_data["강조표시_단계"] = "무가염_조건확인"
                        return make_response(
                            f"""📊 무가염 추가 조건 확인

━━━━━━━━━━━━━━━
📌 입력 함량: {amount}mg (100g당)
━━━━━━━━━━━━━━━
...
🔽 감미료를 사용하나요?""",
                    ...
                    amount = user_data.get("강조표시_함량", 0)

                    for key in ["강조표시_모드", "강조표시_단계", "강조표시_영양소",
                                "강조표시_용어", "강조표시_영양소_목록", "강조표시_함량"]:
...
사용자가 직접 확인해야 합니다.""",
            ...
            # ===== 감미료 확인 단계 (마)(1)) =====
            if step == "감미료_확인":
                if user_input in ["예", "아니요"]:
                    user_data["감미료_사용"] = (user_input == "예")
                    user_data["강조표시_단계"] = "저열량_확인"
                    return make_response(
                        """📊 추가 표시사항 확인
...
🔽 저열량 또는 열량 감소 기준에
   적합한가요?""",
            ...
            # ===== 저열량 확인 단계 (마)(2)) → 당류 최종 결과 =====
            if step == "저열량_확인":
                if user_input in ["예", "아니요"]:
                    low_cal = (user_input == "예")
                    uses_sweetener = user_data.get("감미료_사용", False)
                    amount = user_data.get("강조표시_함량", 0)
                    claim_type = user_data.get("강조표시_용어", "무가당")
                    ...
                    claim_label = "무가당" if claim_type == "무가당" else "무(당류)"
...
━━━━━━━━━━━━━━━
기준을 충족하여
'{claim_label}' 표시가 가능합니다.{additional_text}
...
• 영유아(만1~2세): 탄수화물 150g, 당류 50g,
  단백질 35g, 지방 30g
• 일반(3세 이상): 표준 기준치 적용""",
                ["영유아(만1~2세)", "일반(3세 이상)", "이전", "처음으로"]
            )
        ...
        if user_data.get("표시값변환_모드"):
            ...
            # ===== 연령 선택 단계 =====
            if step == "연령_선택":
                if user_input == "영유아(만1~2세)":
                    user_data["영양표시_연령구분"] = "영유아"
                    user_data["표시값변환_단계"] = "영양소_선택"
                    return make_response(
                        """📊 영양성분 표시값 변환 [영유아(만1~2세)]
...
🔽 영양소를 선택해주세요""",
                ...
                elif user_input == "일반(3세 이상)":
                    user_data["영양표시_연령구분"] = "일반"
                    user_data["표시값변환_단계"] = "영양소_선택"
                    return make_response(
                        """📊 영양성분 표시값 변환 [일반(3세 이상)]
...
🔽 영양소를 선택해주세요""",
            ...
            if step == "영양소_선택":
                ...
                # 기타 영양소 분류
                기타_지방산 = ["리놀레산", "알파-리놀렌산", "EPA와 DHA의 합"]
                기타_비타민 = ["비타민A", "비타민D", "비타민E", "비타민K", "비타민C", "비타민B1", "비타민B2", "나이아신", "비타민B6", "엽산", "비타민B12", "판토텐산", "바이오틴"]
                기타_무기질 = ["칼슘", "인", "칼륨", "마그네슘", "철분", "아연", "구리", "망간", "요오드", "셀레늄", "몰리브덴", "크롬"]
                기타_전체 = 기타_지방산 + 기타_비타민 + 기타_무기질
...
예: 127.3, 2.8, 0.3""",
                ...
                # 기타 영양소 직접 선택 (분류 안 거치고 바로 선택된 경우)
                if user_input in 기타_전체:
                    user_data["표시값변환_영양소"] = user_input
                    user_data["표시값변환_단계"] = "함량_입력"
                    age = user_data.get("영양표시_연령구분", "일반")
                    dv = get_daily_value(user_input, age)
                    dv_text = f"1일 기준치: {dv['daily_value']} {dv['unit']}" if dv else "기준치 정보 없음"
...
예: 127.3, 2.8, 0.3""",
            ...
            if step == "기타_분류_선택":
                기타_지방산 = ["리놀레산", "알파-리놀렌산", "EPA와 DHA의 합"]
                기타_비타민_1 = ["비타민A", "비타민D", "비타민E", "비타민K", "비타민C", "비타민B1", "비타민B2"]
                기타_비타민_2 = ["나이아신", "비타민B6", "엽산", "비타민B12", "판토텐산", "바이오틴"]
                기타_무기질_1 = ["칼슘", "인", "칼륨", "마그네슘", "철분", "아연", "구리"]
                기타_무기질_2 = ["망간", "요오드", "셀레늄", "몰리브덴", "크롬"]

                if user_input == "지방산류":
                    user_data["표시값변환_단계"] = "기타_영양소_선택"
                    user_data["표시값변환_기타_분류"] = "지방산류"
                    return make_response(
                        "📊 지방산류\n\n🔽 영양소를 선택해주세요",
                        기타_지방산 + ["이전", "처음으로"]
                    )
                elif user_input == "비타민류":
                    user_data["표시값변환_단계"] = "기타_영양소_선택"
                    user_data["표시값변환_기타_분류"] = "비타민류"
                    return make_response(
                        "📊 비타민류\n\n🔽 영양소를 선택해주세요",
                        기타_비타민_1 + ["더보기▶", "이전", "처음으로"]
                    )
                elif user_input == "무기질류":
                    user_data["표시값변환_단계"] = "기타_영양소_선택"
                    user_data["표시값변환_기타_분류"] = "무기질류"
                    return make_response(
                        "📊 무기질류\n\n🔽 영양소를 선택해주세요",
                        기타_무기질_1 + ["더보기▶", "이전", "처음으로"]
                    )
                else:
                    return make_response(
                        "❌ 아래 버튼에서 분류를 선택해주세요.",
            ...
            if step == "기타_영양소_선택":
                ...
                기타_무기질_2 = ["망간", "요오드", "셀레늄", "몰리브덴", "크롬"]
                기타_지방산 = ["리놀레산", "알파-리놀렌산", "EPA와 DHA의 합"]
                기타_전체 = 기타_지방산 + 기타_비타민_1 + 기타_비타민_2 + 기타_무기질_1 + 기타_무기질_2
                분류 = user_data.get("표시값변환_기타_분류", "")
                ...
                if user_input == "더보기▶":
                    if 분류 == "비타민류":
                        return make_response(
                            "📊 비타민류 (계속)\n\n🔽 영양소를 선택해주세요",
                            기타_비타민_2 + ["◀이전목록", "이전", "처음으로"]
                        )
                    elif 분류 == "무기질류":
                        return make_response(
                            "📊 무기질류 (계속)\n\n🔽 영양소를 선택해주세요",
                            기타_무기질_2 + ["◀이전목록", "이전", "처음으로"]
                        )
...
예: 127.3, 2.8, 0.3""",
            ...
            # ===== 열량 입력방식 선택 =====
            if step == "열량_입력방식":
                if user_input == "실측값 입력":
                    user_data["표시값변환_단계"] = "함량_입력"
                    rule = get_rounding_rule("열량")
                    rule_text = rule['note'] if rule else "일반 반올림 적용"
                    return make_response(
                        f"""📊 [열량] 표시값 변환
...
예: 127.3, 245, 500""",
                ...
                elif user_input == "표시값으로 산출":
                    user_data["표시값변환_단계"] = "열량_산출유형"
                    return make_response(
                        """📊 [열량] 표시값으로 산출
...
② 당알콜등 별도표시
  ...
  + 단백질×4 + 지방×9""",
            ...
            # ===== 열량 산출유형 선택 (일반 vs 당알콜등 별도표시) =====
            if step == "열량_산출유형":
                if user_input == "일반 산출 ①":
                    user_data["표시값변환_단계"] = "열량_일반_입력"
                    user_data["표시값변환_산출_데이터"] = {}
                    user_data["표시값변환_산출_인덱스"] = 0
                    return make_response(
                        """📊 [열량] 일반 산출 (1/3)
...
예: 25, 50.5, 0""",
                ...
                elif user_input == "당알콜등 별도표시 ②":
                    user_data["표시값변환_단계"] = "열량_별도_입력"
                    user_data["표시값변환_산출_데이터"] = {}
                    user_data["표시값변환_산출_인덱스"] = 0
                    return make_response(
                        """📊 [열량] 당알콜등 별도표시 산출 (1/8)
...
예: 3.5, 0""",
            ...
            if step == "열량_일반_입력":
                ...
                try:
                    val = float(user_input.replace(",", "").strip())
                    idx = user_data.get("표시값변환_산출_인덱스", 0)
                    data = user_data.get("표시값변환_산출_데이터", {})

                    key = 열량_일반_순서[idx][0]
                    data[key] = val
                    user_data["표시값변환_산출_데이터"] = data
                    idx += 1
                    user_data["표시값변환_산출_인덱스"] = idx

                    if idx < len(열량_일반_순서):
                        next_label = 열량_일반_순서[idx][1]
                        return make_response(
                            f"""📊 [열량] 일반 산출 ({idx + 1}/{len(열량_일반_순서)})
...
예: 15, 3.5, 0""",
                        ...
                        # 모든 입력 완료 → 알콜/유기산 추가 여부
                        user_data["표시값변환_단계"] = "열량_추가여부"
                        return make_response(
                            f"""📊 [열량] 일반 산출
...
알콜 또는 유기산이 포함되어 있나요?""",
            ...
            if step == "열량_추가여부":
                data = user_data.get("표시값변환_산출_데이터", {})
                if user_input == "바로 계산":
                    # 열량 계산: 탄수화물×4 + 단백질×4 + 지방×9
                    kcal = data["탄수화물"] * 4 + data["단백질"] * 4 + data["지방"] * 9

                    age = user_data.get("영양표시_연령구분", "일반")
                    result = get_display_value("열량", kcal, age)
                    ...
                    formula = f"({data['탄수화물']}×4) + ({data['단백질']}×4) + ({data['지방']}×9)"
                    response_text = f"""📊 열량 산출 결과

━━━━━━━━━━━━━━━
📌 산출 내역
━━━━━━━━━━━━━━━
• 탄수화물: {data['탄수화물']}g × 4 = {data['탄수화물'] * 4} kcal
• 단백질: {data['단백질']}g × 4 = {data['단백질'] * 4} kcal
• 지방: {data['지방']}g × 9 = {data['지방'] * 9} kcal
...
━━━━━━━━━━━━━━━
🔽 다른 영양소를 선택하거나 이전을 눌러주세요"""

                    return make_response(response_text, ["열량", "탄수화물", "당류", "식이섬유", "단백질", "지방", "포화지방", "트랜스지방", "콜레스테롤", "나트륨", "기타", "이전", "처음으로"])

                elif user_input == "알콜/유기산 추가":
                    user_data["표시값변환_단계"] = "열량_알콜유기산_입력"
                    user_data["표시값변환_산출_인덱스"] = 0
                    return make_response(
                        """📊 [열량] 알콜/유기산 추가 (1/2)
...
예: 5.2, 0""",
            ...
            if step == "열량_알콜유기산_입력":
                ...
                try:
                    ...
                    if idx < len(알콜유기산_순서):
                        next_label = 알콜유기산_순서[idx][1]
                        return make_response(
                            f"""📊 [열량] 알콜/유기산 추가 ({idx + 1}/{len(알콜유기산_순서)})
...
예: 3.0, 0""",
                        ...
                        # 열량 계산: 탄수화물×4 + 단백질×4 + 지방×9 + 알콜×7 + 유기산×3
                        알콜 = data.get("알콜", 0)
                        유기산 = data.get("유기산", 0)
                        kcal = (data["탄수화물"] * 4 + data["단백질"] * 4
                                + data["지방"] * 9 + 알콜 * 7 + 유기산 * 3)
...
• 지방: {data['지방']}g × 9 = {data['지방'] * 9} kcal
• 알콜: {알콜}g × 7 = {알콜 * 7} kcal
• 유기산: {유기산}g × 3 = {유기산 * 3} kcal

━━━━━━━━━━━━━━━
📐 산출식
━━━━━━━━━━━━━━━
({data['탄수화물']}×4)+({data['단백질']}×4)+({data['지방']}×9)+({알콜}×7)+({유기산}×3) = {kcal} kcal
...
🔽 다른 영양소를 선택하거나 이전을 눌러주세요"""
            ...
            if step == "열량_별도_입력":
                ...
                try:
                    ...
                    total = len(별도_순서)
                    if idx < total:
                        next_label = 별도_순서[idx][1]
                        return make_response(
                            f"""📊 [열량] 당알콜등 별도표시 산출 ({idx + 1}/{total})
...
예: 3.5, 0""",
                        ...
                        # 열량 계산 (당알콜등 별도표시)
                        당알콜 = data.get("당알콜", 0)
                        에리스리톨 = data.get("에리스리톨", 0)
                        식이섬유 = data.get("식이섬유", 0)
                        타가토스 = data.get("타가토스", 0)
                        알룰로오스 = data.get("알룰로오스", 0)
                        기타탄수화물 = data.get("기타탄수화물", 0)
                        단백질 = data.get("단백질", 0)
                        지방 = data.get("지방", 0)

                        # 당알콜×2.4 + 에리스리톨×0 + 식이섬유×2 + 타가토스×1.5 + 알룰로오스×0 + 기타탄수화물×4 + 단백질×4 + 지방×9
                        kcal = (당알콜 * 2.4 + 에리스리톨 * 0 + 식이섬유 * 2
                                + 타가토스 * 1.5 + 알룰로오스 * 0
                                + 기타탄수화물 * 4 + 단백질 * 4 + 지방 * 9)
                        ...
                        response_text = f"""📊 열량 산출 결과 (당알콜등 별도표시)

━━━━━━━━━━━━━━━
📌 산출 내역
━━━━━━━━━━━━━━━
• 당알콜: {당알콜}g × 2.4 = {당알콜 * 2.4} kcal
• 에리스리톨: {에리스리톨}g × 0 = 0 kcal
• 식이섬유: {식이섬유}g × 2 = {식이섬유 * 2} kcal
• 타가토스: {타가토스}g × 1.5 = {타가토스 * 1.5} kcal
• 알룰로오스: {알룰로오스}g × 0 = 0 kcal
• 기타탄수화물: {기타탄수화물}g × 4 = {기타탄수화물 * 4} kcal
• 단백질: {단백질}g × 4 = {단백질 * 4} kcal
• 지방: {지방}g × 9 = {지방 * 9} kcal
...
예: 25.3, 50, 0.8""",
                ...
                elif user_input == "차감법으로 산출":
                    user_data["표시값변환_단계"] = "탄수화물_차감_입력"
                    user_data["표시값변환_산출_데이터"] = {}
                    user_data["표시값변환_산출_인덱스"] = 0
                    return make_response(
                        """📊 [탄수화물] 차감법 산출 (1/4)

📐 산출식: 100g - (수분+단백질+지방+회분)
...
예: 65.2, 80, 10.5""",
            ...
            if step == "탄수화물_차감_입력":
                ...
                try:
                    ...
                    total = len(차감_순서)
                    if idx < total:
                        next_label = 차감_순서[idx][1]
                        return make_response(
                            f"""📊 [탄수화물] 차감법 산출 ({idx + 1}/{total})
...
수분({수분}) + 단백질({단백질}) + 지방({지방}) + 회분({회분}) = {합계}g

합계가 100g을 초과하여 탄수화물이 음수({탄수화물}g)가 됩니다.
입력값을 확인해주세요.
...
🔽 다른 영양소를 선택하거나 이전을 눌러주세요""",
                        ...
                        age = user_data.get("영양표시_연령구분", "일반")
                        result = get_display_value("탄수화물", 탄수화물, age)
                        ...
                        response_text = f"""📊 탄수화물 산출 결과 (차감법)
...
🔽 다른 영양소를 선택하거나 이전을 눌러주세요"""
            ...
            if step == "함량_입력":
                try:
                    ...
                    # 표시값 계산
                    age = user_data.get("영양표시_연령구분", "일반")
                    result = get_display_value(nutrient, amount, age)
                    rule = get_rounding_rule(nutrient)
                    dv = get_daily_value(nutrient, age)

                    # 영양소 선택 단계로 복귀 (메뉴 유지)
                    user_data["표시값변환_단계"] = "영양소_선택"
                    user_data.pop("표시값변환_영양소", None)

                    # 단위: daily_value 테이블 기준, 없으면 기본 매핑
                    if dv and dv.get('unit'):
                        display_unit = dv['unit']
                    else:
                        unit_map = {
                            "열량": "kcal",
                            "콜레스테롤": "mg",

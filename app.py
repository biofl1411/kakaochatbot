...
INSPECTION_MENU = {
    ...
    "submenus": {
        ...
        "영양표시 도우미": {
            "title": "영양표시 도우미",
            "buttons": ["표시단위 안내", "강조표시 확인", "기준치 조회", "%기준치 계산", "표시값 변환", "이전", "처음으로"],
            "parent": "영양성분검사"
        },
        "소비기한설정": {
            "title": "소비기한설정",
    ...
    try:
        ...
        if user_input == "이전":
            ...
            if user_data.get("강조표시_모드"):
                ...
                return make_response(
                    "📊 영양표시 도우미\n\n원하시는 기능을 선택해주세요.",
                    ["표시단위 안내", "강조표시 확인", "기준치 조회", "%기준치 계산", "표시값 변환", "이전", "처음으로"]
                )
...
다시 계산하시려면 [당알코올 계산]을 선택하세요."""
        ...
        # ===== 영양성분검사 > 계산도우미 > 표시단위 계산 선택 시 (대화형) =====
        if user_input == "표시단위 계산" and (user_data.get("검사분야_메뉴") == "영양성분검사" or user_data.get("현재_메뉴") == "계산도우미"):
...
🔍 식품군을 선택해주세요."""
        ...
        # ===== 영양표시 도우미 > 표시단위 안내 =====
        if user_input == "표시단위 안내" and user_data.get("현재_메뉴") == "영양표시 도우미":
            response_text = """📊 영양성분 표시단위 안내
...
① 1포장당 (총 내용량당)
   • 총 내용량 100g(ml) 미만인 제품
   • 총 내용량이 1회 섭취참고량 미만인 제품

② 100g(ml)당
   • 100g(ml)당 또는 1회 섭취참고량당 표시 선택 가능
   • 대부분의 제품에 적용 가능

③ 1회 섭취참고량당
   • 식품유형별로 정해진 기준량 사용
   • [계산도우미 > 표시단위 계산]에서 확인 가능

④ 낱개당
   • 낱개 중량이 1회 섭취참고량의 50% 이상인 경우
   • 낱개로 나누어 섭취하는 제품
...
━━━━━━━━━━━━━━━
[계산도우미] > [표시단위 계산]에서
식품유형별 1회 섭취참고량을 확인하실 수 있습니다."""

            return make_response(response_text, ["표시단위 계산", "영양표시 도우미", "이전", "처음으로"])
...
🔽 영양소를 선택해주세요""",
                ["열량", "지방", "포화지방", "콜레스테롤", "나트륨", "당류", "식이섬유", "단백질", "이전", "처음으로"]
            )
        ...
        if user_data.get("강조표시_모드"):
            ...
            # 영양소 선택 단계
            if step == "영양소_선택":
                valid_nutrients = ["열량", "지방", "포화지방", "콜레스테롤", "나트륨", "당류", "식이섬유", "단백질"]
                if user_input in valid_nutrients:
                    ...
                    if claims:
                        claims_text = "\n".join([f"• {c['claim_type']}: {c['condition']}" for c in claims])
                    else:
                        claims_text = "해당 영양소의 강조표시 기준이 없습니다."

                    return make_response(
                        f"""📊 [{user_input}] 강조표시 기준
...
예: 2.5, 15, 0.3""",
            ...
            if step == "함량_입력":
                try:
                    ...
                    # 강조표시 가능 여부 확인
                    applicable = check_nutrient_claim(nutrient, amount)
                    ...
                    if applicable:
                        claims_text = "\n".join([f"✅ {c['claim_type']}: {c['condition']}" for c in applicable])
                        note_texts = [c['note'] for c in applicable if c['note']]
                        note_text = "\n".join([f"※ {n}" for n in note_texts]) if note_texts else ""
...
{note_text}"""
                        ...
                        # 해당 영양소의 모든 기준 표시
                        all_claims = get_all_claims_for_nutrient(nutrient)
                        claims_info = "\n".join([f"• {c['claim_type']}: {c['condition']}" for c in all_claims])

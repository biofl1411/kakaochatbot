            ...
            if ocr_result['success'] and ocr_result['food_type']:
                ...
                if user_data["기능"] == "검사항목":
                    ...
                    if result:
                        ...
                        response_text += "📋 품목제조보고서 또는 허가증/신고증을 확인하시어 다시 검색 바랍니다."
                        if similar:
                            response_text += f"\n\n🔍 유사한 항목: {', '.join(similar)}"
                        return make_response(response_text, ["종료"])
                    ...
                    if result:
                        ...
                        # 이미지에서 추출했지만 DB에 없는 경우 - 업종에 따른 안내
                        ...
                        if user_data["업종"] in ["식품제조가공업", "축산물제조가공업"]:
                            response_text += "📋 품목제조보고서를 확인하시어 다시 검색 바랍니다."
                        else:
                            response_text += "📋 자가품질검사 대상인 식품유형은 식품위생법 시행령에 따라 영업신고증에 표기되어야 합니다.\n\n관련된 내용은 홈페이지 → 사업분야 → 자가품질검사에서 확인 바랍니다."
                        if similar:
                            response_text += f"\n\n🔍 유사한 항목: {', '.join(similar)}"
                        return make_response(response_text, ["종료"])
            else:
                ...
                if user_data["기능"] == "검사주기" and user_data.get("업종"):
                    if user_data["업종"] in ["식품제조가공업", "축산물제조가공업"]:
                        response_text += "📋 품목제조보고서를 확인하시어 다시 검색 바랍니다."
                    else:
                        response_text += "📋 자가품질검사 대상인 식품유형은 식품위생법 시행령에 따라 영업신고증에 표기되어야 합니다.\n\n관련된 내용은 홈페이지 → 사업분야 → 자가품질검사에서 확인 바랍니다."
            ...
            if user_data["기능"] == "검사항목":
                ...
                if len(all_matches) > 1:
                    ...
                    if user_data["실패횟수"] >= 3:
                        ...
                        # 2회 실패 시 이미지 업로드 안내 (Vision API 사용 가능 시)
                        response_text = f"❌ '{food_type}'에 대한 검사 항목을 찾을 수 없습니다.\n\n"
                        response_text += "📷 품목제조보고서 또는 허가증/신고증의 사진을 올려주시면 유형을 확인해 드립니다."
                        if similar:
                            response_text += f"\n\n🔍 유사한 항목: {', '.join(similar)}"
                    elif user_data["실패횟수"] == 1:
                        # 1회 실패 시 서류 확인 안내
                        response_text = f"❌ '{food_type}'에 대한 검사 항목을 찾을 수 없습니다.\n\n"
                        response_text += "📋 품목제조보고서 또는 허가증/신고증의 유형을 확인하여 아래의 제시된 유형이 있을 경우에는 선택해주세요."
                        if similar:
                            response_text += f"\n\n🔍 유사한 항목: {', '.join(similar)}"
                    else:
                        response_text = f"❌ '{food_type}'에 대한 검사 항목을 찾을 수 없습니다.\n\n☆ 다른 식품 유형을 입력하거나, [종료]를 눌러주세요."
                ...
                if len(all_matches) > 1:
                    ...
                    if user_data["실패횟수"] >= 3:
                        ...
                        response_text = f"❌ '{food_type}'에 대한 검사주기를 찾을 수 없습니다.\n\n"
                        if user_data["업종"] in ["식품제조가공업", "축산물제조가공업"]:
                            response_text += "📷 품목제조보고서를 사진으로 올려주시면 유형을 확인해 드립니다."
                        else:
                            response_text += "📷 허가증, 신고증의 사진을 올려주시면 유형을 확인해 드립니다."
                        if similar:
                            response_text += f"\n\n🔍 유사한 항목: {', '.join(similar)}"
                    elif user_data["실패횟수"] == 1:
                        # 1회 실패 시 서류 확인 안내
                        response_text = f"❌ '{food_type}'에 대한 검사주기를 찾을 수 없습니다.\n\n"
                        if user_data["업종"] in ["식품제조가공업", "축산물제조가공업"]:
                            response_text += "📋 품목제조보고서의 유형을 확인하여 아래의 제시된 유형이 있을 경우에는 선택해주세요."
                        else:
                            response_text += "📋 허가증, 신고증의 유형이나 품목을 확인해주시고 아래의 제시된 유형이 있을 경우에는 선택해주세요."
                        if similar:
                            response_text += f"\n\n🔍 유사한 항목: {', '.join(similar)}"
                    else:
                        response_text = f"❌ '{food_type}'에 대한 검사주기를 찾을 수 없습니다.\n\n☆ 다른 식품 유형을 입력하거나, [종료]를 눌러주세요."

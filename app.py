        ...
        if user_data.get("계산_모드") == "배합함량":
            ...
            if step == "총중량_입력":
                try:
                    ...
                    user_data["계산_단계"] = "원재료중량_입력"
                    return make_response(
                        f"✅ 총 중량: {total_weight}g\n\n이제 원재료 중량(g)을 입력해주세요.\n\n예: 60",
                        ["이전", "처음으로"]
                    )
                except ValueError:
                    return make_response(
            ...
            if step == "원재료중량_입력":
                try:
                    ingredient_weight = float(user_input.replace("g", "").replace("G", "").strip())
                    total_weight = user_data.get("총중량")

                    if total_weight == 0:
                        return make_response("❌ 총 중량이 0일 수 없습니다.", ["이전", "처음으로"])

                    percentage = (ingredient_weight / total_weight) * 100
...
━━━━━━━━━━━━━━━
• 총 중량: {total_weight}g
• 원재료 중량: {ingredient_weight}g

━━━━━━━━━━━━━━━
📌 계산 결과
━━━━━━━━━━━━━━━
{ingredient_weight} ÷ {total_weight} × 100 = {percentage:.2f}%

✨ 배합 함량: {percentage:.2f}%

━━━━━━━━━━━━━━━
다른 원재료도 계산하시려면 중량을 입력하세요.
처음부터 다시 하시려면 [배합 함량]을 선택하세요."""
        ...
        if user_data.get("계산_모드") == "당알코올":
            ...
            if step == "원재료함량_입력":
                try:
                    ...
                    return make_response(
                        f"✅ 원재료 당알코올 함량: {ingredient_content}%\n\n수분값/Solid(%)를 입력해주세요.\n\n예: 70.5",
                        ["이전", "처음으로"]
                    )
                except ValueError:
                    return make_response(
...
━━━━━━━━━━━━━━━
• 배합 함량: {blend_ratio}%
• 원재료 당알코올 함량: {ingredient_content}%
